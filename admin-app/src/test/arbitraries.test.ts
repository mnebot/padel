...
describe('Arbitraries - Data Generation', () => {
  ...
  it('generates valid admin dashboard stats', () => {
    fc.assert(
      fc.property(adminDashboardStatsArbitrary, (stats) => {
        expect(stats).toHaveProperty('totalActiveBookings');
        expect(stats).toHaveProperty('pendingRequests');
        expect(stats).toHaveProperty('totalUsers');
        expect(stats).toHaveProperty('totalCourts');
        expect(stats).toHaveProperty('courtUsage');
        expect(stats).toHaveProperty('upcomingLotteries');
        expect(stats.totalActiveBookings).toBeGreaterThanOrEqual(0);
        expect(stats.pendingRequests).toBeGreaterThanOrEqual(0);
        expect(Array.isArray(stats.courtUsage)).toBe(true);
        expect(Array.isArray(stats.upcomingLotteries)).toBe(true);
        return true;
      }),
      { numRuns: 100 }
    );
