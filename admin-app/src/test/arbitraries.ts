...
// Admin-specific arbitraries

// Dashboard stats arbitrary
export const adminDashboardStatsArbitrary = fc.record({
  totalActiveBookings: fc.integer({ min: 0, max: 1000 }),
  pendingRequests: fc.integer({ min: 0, max: 500 }),
  totalUsers: fc.integer({ min: 0, max: 10000 }),
  totalCourts: fc.integer({ min: 0, max: 50 }),
  courtUsage: fc.array(
    fc.record({
      courtId: uuidArbitrary,
      courtName: nameArbitrary,
      bookingCount: fc.integer({ min: 0, max: 1000 }),
    }),
    { minLength: 0, maxLength: 20 }
  ),
  upcomingLotteries: fc.array(
    fc.record({
      date: dateStringArbitrary,
